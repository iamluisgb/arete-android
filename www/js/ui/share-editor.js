  ...
  try {
    ...
    if (navigator.share && navigator.canShare?.({ files: [file] })) {
      ...
      try {
        ...
        const { value } = await Filesystem.getUri({ directory: Directory.Cache, path: fileName });
        await Share.share({ url: value, title: _mode === 'running' ? 'Mi carrera — Areté' : 'Mi entreno — Areté' });
      } catch (e) {
        console.warn('Capacitor share fallback:', e);
